...
import { Activity, Target, Zap, Timer, TrendingUp, BarChart3 } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          <StatCard label="Total Rallies" value={data.totalRallies} color="primary" delay={150} />
          <StatCard label="Avg Rally Length" value={`${data.avgRallyLength}s`} color="blue" delay={200} />
          <StatCard label="Longest Rally" value={`${data.longestRally}s`} color="green" delay={250} />
        </div>
      </div>

      {/* Shot Analysis */}
      <div>
        <div className="flex items-center gap-3 mb-4 animate-slide-up" style={{ animationDelay: "300ms" }}>
          <div className="p-2 rounded-lg bg-primary/10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <StatCard label="Topspin Shots" value={data.topspinShots} color="amber" delay={350} />
          <StatCard label="FH Winners" value={data.forehandWinners} color="green" delay={400} />
          <StatCard label="BH Winners" value={data.backhandWinners} color="blue" delay={450} />
          <StatCard label="Net Points" value={data.netPoints} color="red" delay={500} />
        </div>
      </div>

      {/* Serve */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <StatCard label="Est. Serve Speed" value={data.serveSpeed} subtitle="Based on motion analysis" color="primary" delay={550} />
      </div>
    </div>
  );
};
